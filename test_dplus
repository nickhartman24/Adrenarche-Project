...
def get_first_last_position_per_chromosome(infile):
  ...
  """
  first_last_position_chromosome_dict=defaultdict(None)
  with open(infile,"r") as file:
    for line in file:
      spline=line.strip().split(",")
      try:
        first_last_position_chromosome_dict[spline[0]]=(int(spline[1]),int(spline[2]))
      except ValueError:
        continue
...
parser.add_argument("-haplotype_index"type=str,nargs='?',help="Haplotype index corresponding to introgression maps.")
...
if args.haplotype:
  haplotype_string="_"
  haplotype_indexes=[int(haplotye_index) for haplotype_index in args.haplotype_index.split(haplotype_string)[:2]]
  introgressed_tracts={}
...
while (stop < (last - args.window_size)):
  ...
  for vcf_row in human_iterator:
    ...
    if args.haplotype:
      ...
      data[position]["pop_one_derived_freq"]=get_derived_allele_freq(array=vcf_row[human_individual_index_dict["pop1"]]
